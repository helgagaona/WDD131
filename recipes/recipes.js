...
function filterRecipes(query) {
    const filtered = recipes.filter(recipe => {
        // Convert everything to lowercase and check if the query is in the name, description, or tags
...
function searchHandler(e) {
    ...
    const searchInput = document.querySelector("#search-input").value; // Assuming the search input has id="search-input"

    // Use filterRecipes to filter the recipes based on the query
    const filteredRecipes = filterRecipes(searchInput);

    // Render the filtered and sorted list of recipes
...
function init() {
    ...
    const searchButton = document.querySelector("#search-button"); // Assuming the search button has id="search-button"
    if (searchButton) {
